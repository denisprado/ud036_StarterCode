    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        ...
        <div class="collapse navbar-collapse" id="navbarTogglerDemo01">
            ...
            <li class="nav-item active">
                <a class="nav-link All" href="#">All <span class="sr-only">
                (current)</span></a>
            </li>
            {menu_genres}
            </ul>
    ...
    <div class="modal" id="trailer">
      <div class="modal-dialog">
        <div class="modal-content">
          <button type="button" class="hanging-close" aria-label="Close" data-dismiss="modal"
    ...
    <footer class="p-4 bg-light">
        <div class="row p-4">
            <div class="col text-center p-4">
                Using <img src="powered.png" width="77" height="30"> API
            </div>
        </div>
    </footer>
  </body>
...
movie_tile_content = '''
<div class="movies__item movie-tile {movie_classes}"
...
data-target="#trailer">
    ...
    <h6 class="movies__item__button">
    <span class="badge badge-light">{movie_genres}</span></h6>
    <p class="movies__item__storyline">{movie_storyline}</p>
    <a class="btn btn-outline-primary movies__item__button movie-tile"
    href="#">
    Show&nbsp;Trailer</a>
</div>
'''
...
def create_item_menu(item):
    item_class = item.replace(" ", "")
    li_item = '<li class="nav-item"><a class = "nav-link ' + item_class + '" >'
    item = li_item + item + '</a></li>'
    return str(item)
...
def create_movie_tiles_content(movies):
    ...
    for movie in movies:
        ...
        badge_divisor = '</span><span class="badge badge-light">'
        g_list = map(lambda foo: foo.replace(' ', ''), movie.genres_names)
        ...
        content += movie_tile_content.format(
            ...
            movie_genres=badge_divisor.join(map(str, movie.genres_names)),
            movie_classes=' '.join(map(str, g_list))
        )
    return content
